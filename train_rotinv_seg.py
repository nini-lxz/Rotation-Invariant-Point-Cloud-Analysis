...
os.system('cp train_rotinv_seg.py %s' % (LOG_DIR))  # bkp of train procedure
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            # Get model and loss
            pred, end_points = MODEL.get_model_seg(pointclouds_pl, cls_labels_pl, is_training_pl, bn_decay=bn_decay)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data = batch_data[..., 0:3] # batch_data [B, 2048, 3]
        batch_data = provider.rotate_point_cloud(batch_data)  # rotate around z axis
