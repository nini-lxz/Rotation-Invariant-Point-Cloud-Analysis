...
parser.add_argument('--model', default='model', help='Model name [default: pointnet2_part_seg]')
parser.add_argument('--model_path', default='log_y_seg/model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
def evaluate(vote_num):
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            pointclouds_pl, labels_pl, cls_labels_pl = MODEL.placeholder_inputs_seg(BATCH_SIZE, NUM_POINT)
            is_training_pl = tf.placeholder(tf.bool, shape=())
            print is_training_pl
            
            print "--- Get model and loss"
            pred, end_points = MODEL.get_model_seg(pointclouds_pl, cls_labels_pl, is_training_pl)
            loss = MODEL.get_loss_seg(pred, labels_pl)
        ...
        eval_one_epoch(sess, ops, vote_num)
...
def eval_one_epoch(sess, ops, vote_num):
    ...
    best_miou = 0.0
    best_shape_ious = {cat: [] for cat in seg_classes.keys()}
    for vote_idx in range(vote_num):
        ...
        for batch_idx in range(num_batches):
            ...
            cur_batch_data = cur_batch_data[..., 0:3]
            ...
            batch_data = provider.rotate_point_cloud_so(batch_data)  # rotate around random axis
        ...
        if mean_shape_ious >= best_miou:
            best_miou = mean_shape_ious
            best_shape_ious = shape_ious

    for cat in sorted(best_shape_ious.keys()):
        log_string('eval mIoU of %s:\t %f'%(cat, best_shape_ious[cat]))

    log_string('eval mean mIoU: %f' % (best_miou))
